...
export async function queryEntities(entity: string, params: Record<string, string> = {}) {
...
export async function getQueryTaskStatus(taskId: string): Promise<QueryTaskStatus> {
  const response = await fetch(`${BASE_URL}/queries/tasks/${taskId}`, { headers: { ...authHeaders() } });

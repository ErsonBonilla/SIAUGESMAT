...
export default function Layout({ children, title }: LayoutProps) {
  ...
  return (
    <div class="flex min-h-screen w-full text-[var(--text-primary)]">
      <Sidebar mobileOpen={mobileOpen.value} onClose={() => (mobileOpen.value = false)} />
      ...
      <div class="md:hidden h-14 flex items-center px-4 sticky top-0 z-20 border-b border-[var(--border-secondary)] bg-[var(--navbar-bg)] text-[var(--navbar-text)]">
        ...
        <div class="ml-auto flex items-center gap-2">
          {profileSignal.value?.profileimageurl
            ? (
              <img src={profileSignal.value.profileimageurl} alt="" class="w-8 h-8 rounded-full object-cover" />
            )
            : (
              <div class="w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs shrink-0"
                style={{ backgroundColor: "var(--navbar-user-bg)", color: "var(--navbar-user-text)" }}>

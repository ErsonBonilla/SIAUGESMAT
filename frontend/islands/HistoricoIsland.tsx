...
export default function HistoricoIsland() {
  ...
  useEffect(() => {
    async function loadHistory() {
      ...
      try {
        ...
        error.value = e instanceof Error ? e.message : "Error al cargar histórico.";
      } finally {
        loading.value = false;
      }
    }
  ...
  const inactiveLight = "bg-[var(--bg-primary)] border-[var(--border-secondary)]";
  const inactiveDark = "bg-[var(--bg-tertiary)] border-[var(--border-secondary)] text-[var(--text-secondary)]";
        ...
        {(["history", "comparison"] as const).map((v) => (
          <button
            ...
            class={`px-4 py-2 rounded-lg text-sm font-medium cursor-pointer transition-colors duration-200 ${
              view.value === v
                ? activeBtn
                : `${inactiveBase} ${darkSignal.value ? inactiveDark : inactiveLight}`
            }`}
          >
              ...
              <>
                <div class="mb-4 flex justify-center">
                  <SemesterMultiPicker
                    ...
                    availableSemesters={historyData.value.map((d) => d.semester)}
                  />
                </div>
                <MetricsChart

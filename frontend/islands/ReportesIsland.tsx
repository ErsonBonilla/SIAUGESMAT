...
export default function ReportesIsland({ executionId }: Props) {
  ...
  const charts = Object.entries(CHART_NAMES).map(([id, title]) => ({ id, title }));
  ...
  return (
    <>
      {reports.value && reports.value.reports.length > 0 && (
        <section class="mb-10">
          <div class="flex items-center justify-between mb-4">
            <h2 class="text-lg font-semibold text-[var(--text-primary)]">Descargar reportes</h2>
            <button
              onClick={() => handleDownload(
                `${BASE_URL}/reports/${executionId}/reports/download`,
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {(reports.value.reports as ReportInfo[]).map((r) => {
              ...
              return (
                <button
                  onClick={() => handleDownload(
                    `${BASE_URL}/reports/${executionId}/reports/${r.name}.csv`,
                    r.filename,
                  )}
                  disabled={downloading.value !== null}
                ...
                >
                  <div class="flex items-center gap-2">
                    <span>📄</span>
                    <span class="flex-1 text-sm font-medium text-[var(--text-primary)] truncate">{label}</span>
                    {isDownloading
                      ? <span class="inline-block w-4 h-4 border-2 border-[var(--accent)] border-t-transparent rounded-full animate-spin shrink-0" />
                      : <span class="text-xs text-[var(--text-secondary)] shrink-0">{formatSize(r.size)}</span>}
                  </div>
                </button>
              );
            })}
      ...
      <section>
        <div class="mb-4">
          <h2 class="text-lg font-semibold text-[var(--text-primary)]">Gráficos</h2>
          <p class="text-[var(--text-secondary)] text-sm">Visualización de datos generados con Plotly.js</p>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          {charts.map(({ id, title }) => (
            <Chart key={id} executionId={executionId} chartName={id} title={title} height="380px" />
          ))}
        </div>
      </section>
    </>

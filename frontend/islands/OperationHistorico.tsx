...
import { getOperationsAnalytics, type OperationsHistoryItem } from "../services/api.ts";
...
const METRICS = [
  ...
  { key: "categories_deleted",   name: "Categorías eliminadas",    color: "#F05458" },
...
export default function OperationHistorico({ entityType, action }: Props) {
  ...
  useEffect(() => {
    (async () => {
      try {
        data.value = await getOperationsAnalytics(undefined, 12, entityType, action);
      } catch (e) {
        error.value = e instanceof Error ? e.message : "Error al cargar analítica";
      } finally {
        loading.value = false;
      }
    })();
  ...
  useEffect(() => {
    if (view.value !== "history" || !data.value.length || !containerRef.current) return;
    let cancelled = false;
    ...
    (async () => {
      try {
        ...
        if (!cancelled && containerRef.current) {
          containerRef.current.innerHTML = `<div class="text-red-500 text-center p-4">Error al cargar gráfico</div>`;
        }
      }
    })();
    ...
    return () => {
      ...
      if (containerRef.current) {
        try { window.Plotly?.purge(containerRef.current); } catch { /* ignore */ }
      }
    };
  }, [darkSignal.value, data.value, view.value]);

  const activeBtn = "bg-[var(--accent)] text-white";
  const inactiveBtn = "border bg-[var(--bg-primary)] border-[var(--border-secondary)] text-[var(--text-secondary)]";
      ...
      <div class="bg-[var(--bg-primary)] rounded-xl shadow-sm border border-[var(--border-primary)] p-6">
        {loading.value ? (
          ...
          <div class="text-center py-12 text-[var(--text-secondary)]">
            ...
            <p class="text-sm">Una vez se ejecuten operaciones masivas (crear o eliminar usuarios, categorías, cursos), las métricas aparecerán aquí.</p>
          </div>
        ) : view.value === "history" ? (
              ...
              <tbody>
                {data.value.map((d) => (
                  <tr class="border-b border-[var(--border-primary)] hover:bg-[var(--bg-tertiary)]">
                    ...
                    {relevant.map((m) => (
                      <td key={m.key} class={`py-2 px-2 text-right ${
                        m.key === "total_errors" ? "text-[var(--text-secondary)]" :
                        m.key.includes("deleted") ? "text-[var(--brand-red)]" : "text-[var(--brand-green)]"
                      }`}>{(d as any)[m.key]}</td>
                    ))}
                  </tr>
                ))}

...
function downloadCsv(items: NovedadItem[]) {
  const headers = [
    ...
    "profesor_anterior", "profesor_nuevo",
    "cedula_anterior", "cedula_nueva",
  ];
            ...
            <table class="w-full text-sm">
              <thead>
                <tr class="border-b border-[var(--border-primary)]">
                  <th class="text-left py-2 px-2 text-[var(--text-secondary)] font-medium">Tipo</th>
                  <th class="text-left py-2 px-2 text-[var(--text-secondary)] font-medium">CAT</th>
                  <th class="text-left py-2 px-2 text-[var(--text-secondary)] font-medium">Programa</th>
                    ...
                    <tr class="border-b border-[var(--border-primary)] hover:bg-[var(--bg-secondary)]/50">
                      <td class="py-2 px-2">
                        <span class={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${
                          ...
                          {n.action === "cambio_profesor" ? "Cambio profesor" : n.action === "curso_eliminado" ? "Eliminado" : "Nuevo"}
                        </span>
                      </td>
                      <td class="py-2 px-2 text-[var(--text-primary)] font-mono text-xs">{parts[0] || "—"}</td>
                      <td class="py-2 px-2 text-[var(--text-primary)] font-mono text-xs">{parts[1] || "—"}</td>
                      <td class="py-2 px-2">
                        <div class="font-medium text-[var(--text-primary)] text-sm">{n.course_fullname || n.base_key}</div>
                        <div class="text-xs text-[var(--text-muted)] font-mono">{n.old_shortname || n.new_shortname}</div>
                      </td>
                      <td class="py-2 px-2 text-[var(--text-secondary)] text-sm">
                        {n.old_prof_name || n.old_prof_cedula || <span class="italic text-xs">—</span>}
                      </td>
                      <td class="py-2 px-2 text-[var(--text-secondary)] text-sm">
                        {n.new_prof_name || n.new_prof_cedula || <span class="italic text-xs">—</span>}
                      </td>
                    </tr>
                  );
                })}

...
Deno.test("Chart - renderiza contenedor con título", () => {
  ...
  const html = render(<Chart executionId={1} chartName="resumen_operaciones" title="Resumen de operaciones" />);
...
Deno.test("Chart - renderiza sin título", () => {
  darkSignal.value = false;
  const html = render(<Chart executionId={2} chartName="distribucion_incidencias" />);
...
Deno.test("Chart - usa height personalizado", () => {
  ...
  const html = render(<Chart executionId={3} chartName="cursos_por_accion" height="500px" />);

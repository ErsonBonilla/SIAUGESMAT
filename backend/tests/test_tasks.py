...
class TestProcessEtlFile:
    ...
    def test_happy_path(
        self, mock_mark, mock_etl, mock_get_ms, mock_integ, mock_run_async, mock_delay
    ):
    ...
    def test_moodle_overloaded_re_raised(self):
        with (
            ...
            patch(
                "app.workers.tasks.ETLService.process",
                side_effect=MoodleOverloadedError("overloaded"),
            ),
        ):
            mock_ex = MagicMock()
            mock_ex.status = "running"

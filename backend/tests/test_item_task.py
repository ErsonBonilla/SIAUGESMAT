...
class TestProcessEtlItem:
    ...
    def test_create_course_success(self):
        with patch("app.workers.phases.item_task.SessionLocal") as mock_sl, \
            ...
            mock_integ_cls.return_value = mock_integ
            item = _make_item(action="create", fullname="New Course", category_idnumber="CAT_01", template_id=42)
            mock_get.return_value = item
            mock_sl.return_value = MagicMock()
            process_etl_item(1)
    ...
    def test_create_user_success(self):
        with patch("app.workers.phases.item_task.SessionLocal") as mock_sl, \
            ...
            mock_integ_cls.return_value = mock_integ
            item = _make_item(action="create_user", firstname="John", lastname="Doe",
                              email="john@test.com", password="pwd123", cedula="123",
    ...
    def test_enrol_success(self):
        with patch("app.workers.phases.item_task.SessionLocal") as mock_sl, \
            ...
            mock_integ.enrol_teacher.assert_called_once_with(
                username="IDENTIFIER_001", course_shortname="CURSE_101",
